...
class SimplificationContext private(parent: SimplificationContext) {
  ...
  def constant(i: Int): Node = {
    constant(i match {
      case 0 => false
      case 1 => true
      case other => throw new IllegalArgumentException(s"Value for constants must be either 0 or 1! Provided " +
        s"value: $other")
    })
  }
