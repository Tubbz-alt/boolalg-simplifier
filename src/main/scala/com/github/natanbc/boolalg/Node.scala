...
object Node {
    ...
    implicit val _nodeOrdering: Ordering[Node] = (x, y) => {
        (x, y) match {
            ...
            case (ConstNode(_), AndNode(_)) | (ConstNode(_), OrNode(_)) | (ConstNode(_), NegNode(_)) | (ConstNode(_), InputNode(_)) => 1
            case (ConstNode(a), ConstNode(b)) => (if(b) 1 else 0) - (if(a) 1 else 0)
        }
    }
